...
const DashboardContent = () => {
    ...
    const fetchRealTimeMetrics = async () => {
        try {
            const data: RealTimeMetrics = await authorizedFetch('/conversations/real-time-metrics/');
            setRealTimeData(data);
        } catch (err) {
            setError(err instanceof Error ? err.message : 'Failed to fetch real-time metrics');
        }
    ...
    const fetchStatusOverview = async () => {
        try {
            const data: StatusOverview = await authorizedFetch('/conversations/status-overview/');
            setStatusData(data);
        } catch (err) {
            setError(err instanceof Error ? err.message : 'Failed to fetch status overview');
        }
