...
// Main page component - Fixed state synchronization
export default function ConversationsPage({ onUnreadCountChange }: { onUnreadCountChange?: (count: number) => void } = {}) {
   ...
   const [totalUnreadCount, setTotalUnreadCount] = useState<number>(0);
                ...
                try {
                    ...
                    if (response.type === 'conversation_list' && response.data?.results) {
                        ...
                        if (response.conversation) {
                            ...
                            // ✅ FIX: Only update selectedConversation if it matches AND the data is different
                            setSelectedConversation(prev => {
                                if (prev && prev.id === updatedConv.id) {
                                    // Only update if there are actual changes to prevent unnecessary re-renders
                                    const hasChanges =
                                        prev.ai_enabled !== updatedConv.ai_enabled ||
                                        prev.current_handler_type !== updatedConv.current_handler_type ||
                                        prev.customer_name !== updatedConv.customer_name ||
                                        prev.platform_name !== updatedConv.platform_name;

                                    if (hasChanges) {
                                        console.log('🔄 Updating selected conversation from WebSocket:', updatedConv);
                                        return updatedConv;
                                    }
                                }
                                return prev;
    ...
    // ✅ FIXED: Improved toggleAIControl with better state management
    const toggleAIControl = async (): Promise<void> => {
        if (!selectedConversation) {
            console.warn('No selected conversation for AI toggle');
            return;
        }
        ...
        try {
            ...
            // ✅ FIX: Update selectedConversation FIRST, then conversations array
            setSelectedConversation(updatedConv);

            // Update conversations list
            setConversations(prev => {
                const updated = prev.map(c => c.id === updatedConv.id ? updatedConv : c);
                console.log('📝 Updated conversations list after AI toggle');
                return updated;
            });
    ...
    // KEEP ONLY this useEffect (around line 635-645):
    useEffect(() => {
        if (selectedConversation && conversations.length > 0) {
            const updatedSelected = conversations.find(c => c.id === selectedConversation.id);
            if (updatedSelected &&
                (updatedSelected.ai_enabled !== selectedConversation.ai_enabled ||
                    updatedSelected.current_handler_type !== selectedConversation.current_handler_type)) {
                console.log('🔄 Syncing selected conversation with updated data');
    ...
    const mapApiConversationToState = (apiConv: ApiConversation): Conversation => {
        return {
            id: apiConv.id,
            customer_name: apiConv.customer_name || 'Unknown',
            customer_platform_username: apiConv.customer_platform_username || '',
            platform_name: apiConv.platform_name || 'Unknown Platform',
            account_name: apiConv.account_name || '',
            current_handler_type: apiConv.current_handler_type || 'human',
            ai_enabled: apiConv.ai_enabled ?? false,
            status: apiConv.status || '',
            priority: apiConv.priority || '',
            message_count: apiConv.message_count || 0,
            unread_count: apiConv.unread_count || 0,
            last_message_preview: apiConv.last_message_preview || '',
            time_since_last_message: apiConv.time_since_last_message || 0,
